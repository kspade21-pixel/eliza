...
export interface BacktestPolicy {
  ...
  feeBps: bigint; spreadBps: bigint; marketImpactBps: bigint;
  fastWindow: number; slowWindow: number;
}
export interface ScenarioResult {
  name: "optimistic" | "base" | "stress"; feeBps: string; spreadBps: string;
  marketImpactBps: string; costProvenance: "illustrative-derived" | "illustrative-policy-input";
  ...
  netVsCashBps: string; netVsBuyHoldBps: string;
...
export interface BacktestResult {
  ...
  runManifest: { schemaVersion: "paper-backtest-run-manifest/v2";
    ...
    venueBasis: "none-observed"; profitabilityRankingPermitted: false;
    drawdownConvention: "liquidation-value-at-each-observation";
    costModel: Array<{ name: string; feeBps: string; spreadBps: string;
      marketImpactBps: string; provenance: string }> };
  endingEquityMicros: string; returnBps: string; inputHash: string;
  algorithmVersion: "sma-5-20-next-observation-cost-model-v3"; asOfMs: number;
}
...
export const DEFAULT_BACKTEST_POLICY: BacktestPolicy = Object.freeze({
  ...
  minimumReserveMicros: 10_000_000n, feeBps: 10n, spreadBps: 10n,
  marketImpactBps: 10n,
...
function scenarios(policy: BacktestPolicy) {
  ...
  return [
    { name: "optimistic", feeBps: scaled(policy.feeBps, 1n, 2n),
      ...
      assumptions: "Illustrative sensitivity at 0.5x base costs; no observed venue basis." },
    { name: "base", feeBps: policy.feeBps, spreadBps: policy.spreadBps,
      marketImpactBps: policy.marketImpactBps,
      costProvenance: "illustrative-policy-input",
      assumptions: "Illustrative caller/default policy inputs; no observed venue basis." },
    { name: "stress", feeBps: scaled(policy.feeBps, 5n, 2n),
      spreadBps: scaled(policy.spreadBps, 5n, 2n),
      marketImpactBps: scaled(policy.marketImpactBps, 5n, 2n),
      costProvenance: "illustrative-derived",
      assumptions: "Illustrative sensitivity at 2.5x base costs; not a worst-case bound and no observed venue basis." },
  ] as const;
}
...
function scenario(input: readonly HistoricalPrice[], policy: BacktestPolicy, s: ReturnType<typeof scenarios>[number]): ScenarioResult {
  const p = { ...policy, feeBps: s.feeBps, spreadBps: s.spreadBps,
    marketImpactBps: s.marketImpactBps };
  let cash = p.initialCashMicros, quantity = 0n, trades = 0, peak = cash, drawdown = 0n;
  let holdPeak = p.initialCashMicros, holdDrawdown = 0n;
  ...
  for (let i = p.slowWindow; i < input.length; i++) {
    ...
    const holdDd = holdPeak > 0n ? ((holdPeak - holdEquity) * BPS_SCALE) / holdPeak : 0n;
  ...
  return { name: s.name, feeBps: p.feeBps.toString(), spreadBps: p.spreadBps.toString(),
    marketImpactBps: p.marketImpactBps.toString(), costProvenance: s.costProvenance,
...
export function runPaperBacktest(
  ...
  const normalized = { schemaVersion: "paper-backtest-run-manifest/v2", algorithmVersion,
    ...
    scenarios: scenarios(policy).map(({ name, feeBps, spreadBps, marketImpactBps, costProvenance, assumptions }) =>
      ({ name, feeBps: feeBps.toString(), spreadBps: spreadBps.toString(),
        marketImpactBps: marketImpactBps.toString(), costProvenance, assumptions })),
  ...
  const scenarioResults = scenarios(policy).map((s) => scenario(input, policy, s));
  const base = scenarioResults[1]!;
  ...
  return { mode: "PAPER_BACKTEST", researchStatus: "UNVERIFIED_RESEARCH", bars: input.length,
    ...
    finalSignal, warnings, coverage: metrics, scenarios: scenarioResults,
    runManifest: { schemaVersion: "paper-backtest-run-manifest/v2", algorithmVersion,
      executionSemantics: "signal-from-prior-observations; execute-at-next-observation-price",
      priceFieldSemantics: "observation-price; not asserted to be market open",
      ...
      venueBasis: "none-observed", profitabilityRankingPermitted: false,
      drawdownConvention: "liquidation-value-at-each-observation",
      costModel: scenarios(policy).map((item) => ({ name: item.name,
        feeBps: item.feeBps.toString(), spreadBps: item.spreadBps.toString(),
